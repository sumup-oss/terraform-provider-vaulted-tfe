...
func testAccVariableCheckUpdate(
	...
	return func(state *terraform.State) error {
		resourceState := state.Modules[0].Resources["vaulted-tfe_variable.secret"]
...
func TestResourceVariableIntegration_with_VAULTED_PRIVATE_KEY_PATH(t *testing.T) {
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			tmpDir := testutils.TestDir(t, "provider-vaulted-tfe")
			...
			key := acctest.RandomWithPrefix("vaulted-tfe")
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			providerFactoriesWithSpy := map[string]func() (*schema.Provider, error){
				"vaulted-tfe": func() (*schema.Provider, error) {
...
func testResourceVariableConfig(key, value string, category string, sensitive bool) string {
	res := fmt.Sprintf(
		`resource vaulted-tfe_variable secret {
	key          = "%s"
	value        = "%s"
	category     = "%s"
	workspace_id = "%s"
